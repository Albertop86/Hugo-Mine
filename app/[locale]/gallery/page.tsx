...
export default function GalleryPage() {
  ...
  const [generatedSlugs, setGeneratedSlugs] = useState<Set<string>>(new Set())
  ...
  useEffect(() => {
    Promise.all([
      ...
      setGeneratedSlugs(new Set(slugs))
  ...
  // Only show characters with an available skin (premade file or already generated by cron)
  const availableChars = CHARACTERS.filter(c => c.skinFile || generatedSlugs.has(c.slug))
  const filteredChars = activeCharCategory === 'all'
    ? availableChars
    : availableChars.filter(c => c.category === activeCharCategory)
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-10 flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <h2 className="font-extrabold text-lg" style={{ color: 'var(--color-earth)' }}>
            🎮 {charsTitle} ({availableChars.length})
          </h2>
          <div className="flex flex-wrap gap-2">
